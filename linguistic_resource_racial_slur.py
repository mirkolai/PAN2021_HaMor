...
class RacialSlur(object):
    ...
    def __init__(self,language):
        print("Loading Racial Slur for "+language+"...")
        ...
        file_name="external resources/"+language+"/The Racial Slur Database/rsdb.csv"
        spamreader = csv.reader(open(file_name), delimiter=',', quotechar='"')
        for row in spamreader:
            #print(row)
            category=row[1]
            if category in ['blacks',
                           'asians',
                           'mixed races',
                           'jews',
                           'arabs',
    ...
    def get_text_racial_slur_profile(self, text):
    ...
    def get_user_racial_slur_profile(self, texts):
        values=[]
        for text in texts:
...
if __name__ == '__main__':
    ...
    racial_slur = RacialSlur("en")
    print("en")
    print(racial_slur.word_category.keys())
    concepts, values=racial_slur.get_text_racial_slur_profile("@tedcruz And, #HandOverTheServer she wiped clean + 30k deleted emails, explains dereliction of duty/lies re #Benghazi,etc #tcot #SemST")
    print("concepts", "values")
    print(concepts, values)
    texts=["@tedcruz And, hebro #HandOverTheServer homey she wiped clean + 30k deleted emails, explains dereliction of duty/lies re #Benghazi,etc #tcot #SemST",
           "All these fears about safety but a total refusal to address the problem is NONWHITE IMMIGRANTS"]
    concepts, values=racial_slur.get_user_racial_slur_profile(texts)
    print("concepts", "values")
    print(concepts)
    print(values)

    """racial_slur_es = RacialSlur("es")
    print("es")
    concepts, values=racial_slur_es.get_text_racial_slur_profile("RT #USER#: Felices 119 años, #USER#. Historia que tú hiciste, historia por hacer. Hala Madrid!!❤️ #URL#")
    print("concepts", "values")
    print(concepts, values)
    ...
    concepts, values=racial_slur_es.get_user_racial_slur_profile(texts)
    print("concepts", "values")
    print(concepts, values)"""
